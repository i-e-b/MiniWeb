...
public class AssetLoader {
    ...
    public InputStream open(String path) throws IOException {
        ...
        if (HotReloadMonitor.TryLoadFromHost) {
            try {
                InputStream is = EmulatorHostCall.queryHostForData(App.str(R.string.path_assets) + path);
                if (is != null) return is;
            } catch (Exception e){
                Log.w(TAG, App.str(R.string.err_host_load_fail)+App.str(R.string.tag_err)+e);
            }
        }
